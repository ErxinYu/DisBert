...
"""
def sentence2tenorIndex(sentence, vob, indexers): 
    d = {}
    sentence = clean_tokenize_truncate(sentence,178)
    sentence = ["<SOS>"] + sentence + ["<EOS>"]
    d['sent1'] = TextField(list(map(Token, sentence)), token_indexers=indexers)
    c = Instance(d)
    c.index_fields(vob)
    tenorIndex = c.as_tensor_dict()["sent1"]["words"]["tokens"] 
    if tenorIndex.size()[0] < 180:
        num_pad = 180-tenorIndex.size()[0]
        pad_ = torch.zeros(num_pad).long()
        tenorIndex = torch.cat((tenorIndex, pad_))
    else:
        tenorIndex = tenorIndex[:180]
    if tenorIndex.size()[0] != 180:
        print("tenorIndex",tenorIndex.size())
        exit()
    return tenorIndex
...
class mrpc(Task):
    def __init__(self):
        ...
        self.file_by_split = dict(
            train="mrpc/train.csv",
            val="mrpc/validation.csv",
            test="mrpc/test.csv",
        )
        self.max_length = 400
...
class stsb(Task):
    def __init__(self):
        ...
        self.class_number = 14
        self.file_by_split = dict(
            train="stsb/train.csv",
            val="stsb/validation.csv",
            test="stsb/test.csv",
        )
        self.max_length = 400
        print("stsb task............")
...
class SemEvalA(Task):
    def __init__(self):
        ...
        self.class_number = 14
        self.file_by_split = dict(
            train="SemEvalA/train.csv",
            val="SemEvalA/validation.csv",
            test="SemEvalA/test.csv",
        )
        self.max_length = 400
        print("SemEvalA task............")
...
class SemEvalB(Task):
    def __init__(self):
        ...
        self.class_number = 14
        self.file_by_split = dict(
            train="SemEvalB/train.csv",
            val="SemEvalB/validation.csv",
            test="SemEvalB/test.csv",
        )
        self.max_length = 400
        print("SemEvalB task............")
...
class SemEvalC(Task):
    def __init__(self):
        ...
        self.class_number = 14
        self.file_by_split = dict(
            train="SemEvalC/train.csv",
            val="SemEvalC/validation.csv",
            test="SemEvalC/test.csv",
        )
        self.max_length = 400
        print("SemEvalC task............")
...
def get_task(config):
    ...
    if not os.path.exists(task_pkl_path):
        ...
        task.train_data = task_class.process_split(task.train_data_text, indexers, vocab)
        task.validation_data = task_class.process_split(task.val_data_text, indexers, vocab)
